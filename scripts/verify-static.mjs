...
assert.match(weekNfl2, /Tracked Week 2 record: 16–17/);
assert.doesNotMatch(weekNfl2, /still open/);
assert.match(weekNfl2, /Giants at Rams/);
assert.match(weekNfl2, /Rams beat Giants/);

const nflSlate = await readFile(path.join(out, "nfl/index.html"), "utf8");
assert.match(nflSlate, /data-page-type="league"/);
assert.match(nflSlate, /NFL Week 2: who called it/);
