...
import { isLegacyReportedSelection } from "@/lib/evidence";
...
export function PickReport({ report: r }: { report: Report }) {
  return r.week === 2 ? <WeekTwoReport report={r} /> : <WeekOneReport report={r} />;
}

function WeekOneReport({ report: r }: { report: Report }) {
  ...
  return <article className={s.report} aria-labelledby="pick-report-title">
    ...
    <ReportMethod report={r} />
  </article>;
}

function UnderdogTable({ report: r }: { report: Report }) {
  return (
    <section className={s.section} aria-labelledby="report-underdogs">
      <div className={s.sectionHead}><h2 id="report-underdogs">Which underdog picks won in Week {r.week}?</h2></div>
      <div className={s.tableWrap}><table className={s.table}>
        <caption>Tracked Week {r.week} underdogs · 2026 · prices are dated snapshots</caption>
...
function ReportMethod({ report: r }: { report: Report }) {
  return (
    <section id="report-method" className={s.method} aria-labelledby="report-method-title"><h2 id="report-method-title">How we counted the picks</h2><p>We used the published game-winner picks for college football Week {r.week} of the 2026 season. All {r.picks.length} were graded. Each pundit counts once per game. Other weeks, NFL, season-long predictions, tentative commentary and takes without a specific tracked event and side are excluded.</p><p>For this report, underdogs are picked sides below 50¢ in the displayed dated Kalshi snapshot; favorites are above 50¢. No selection was missing a price or exactly 50¢. These shared event snapshots are not closing odds or necessarily the prices when a prediction was made. The records measure straight-up results, not spread covers or betting returns.</p><Link href="/methodology/">How Pundits.Pro verifies and grades picks ↗</Link></section>
  );
}

function WeekTwoReport({ report: r }: { report: Report }) {
  const michigan = r.dogGames.find(g => g.event.slug === "oklahoma-at-michigan-2026")!;
  const winners = michigan.picks;
  const oklahoma = r.favorites.picks.filter(p => p.event.slug === michigan.event.slug);
  const ohio = r.dogGames.find(g => g.event.slug === "ohio-state-at-texas-2026")!;
  const portnoyMiss = ohio.picks.find(p => p.pundit.id === "portnoy")!;
  const favoriteWins = r.favoriteGames.filter(g => g.hit).length;
  const texasHits = r.favorites.picks.filter(p => p.event.slug === ohio.event.slug).length;
  const portnoyDogs = r.underdogs.picks.filter(p => p.pundit.id === "portnoy");
  return <article className={s.report} aria-labelledby="pick-report-title">
    <header className={s.hero}>
      <div className={s.masthead}><span>College football pick report</span><span>{r.season} / WEEK 02</span></div>
      <div className={s.heroGrid}><div>
        <h1 id="pick-report-title"><span className={s.eyebrow}>College football Week {r.week} results: </span>Howard and Portnoy<br /><em>called Michigan.</em></h1>
        <p className={s.dek}>{oklahoma.length} tracked pundits picked Oklahoma. Michigan won {michigan.score}, giving Desmond Howard and Dave Portnoy the week’s only underdog hits.</p>
      </div></div>
      <div className={s.byline}><span>Analysis by <Link href="/about/">Pundits.Pro</Link></span><span>{r.picks.length} picks · {r.punditCount} pundits · {r.games.length} games</span><a href="#report-method">How we counted ↗</a></div>
    </header>

    <section className={s.section} aria-labelledby="report-split">
      <div className={s.sectionHead}><h2 id="report-split">One upset, {michigan.picks.length + oklahoma.length} records</h2></div>
      <div className={s.prose}>
        <p>Michigan’s win changed {michigan.picks.length + oklahoma.length} tracked records at once: {michigan.picks.length} hits for the underdog’s backers and {oklahoma.length} misses for Oklahoma’s. Every other favorite we covered won. Together, our Week {r.week} picks finished {r.hits}–{r.misses} on outright winners.</p>
        <p>The favorite selections went {r.favorites.hits}–{r.favorites.misses}; underdogs went {r.underdogs.hits}–{r.underdogs.misses}. Those are {r.picks.length} picks across just {r.games.length} games. Michigan accounts for both underdog hits and every favorite miss, so the totals reflect one shared result many times.</p>
      </div>
      <figure className={s.comparison}><div className={s.chartGrid}><ResultChart label="Favorites" group={r.favorites} /><ResultChart label="Underdogs" group={r.underdogs} /></div><figcaption>Straight-up pick results, not against the spread. Groups use dated Kalshi snapshots; see the counting method below.</figcaption></figure>
    </section>

    <section className={s.receipt} aria-labelledby="report-receipt">
      <div className={s.receiptIntro}>
        <h2 id="report-receipt">Who picked Michigan over Oklahoma?</h2>
        <p>Howard and Portnoy backed Michigan, priced at {michigan.cents}¢ in our {michigan.event.sourcedAt} snapshot. On the other side: {oklahoma.map(p => p.pundit.name).join(", ")}.</p>
        <p>Both picks are preserved below, with the original source and result a click away.</p>
      </div>
      <div className={s.receiptCard}>
        {winners.map(p => <div key={p.call.id}>
          <div className={s.face}><PunditAvatar src={p.pundit.photo} alt={p.pundit.name} size="feed" /><div><strong>{p.pundit.name}</strong><span>{p.pundit.outlet}</span></div><span className={s.hit}>✓ Hit</span></div>
          <p className={s.source}>{isLegacyReportedSelection(p.call) ? "Reported selection" : "Spoken quote"} · {p.call.source}<br />{p.call.sourceDate}</p>
          <Link className={s.receiptLink} href={takePath(p.event.slug, p.pundit.id)}>Read {p.pundit.name}’s receipt <span>↗</span></Link>
        </div>)}
        <div className={s.receiptNumbers}><div><small>Frozen snapshot</small><strong>{michigan.cents}¢</strong><span>as of {michigan.event.sourcedAt}</span></div><div><small>Final score</small><strong>{michigan.score}</strong><span>Michigan over Oklahoma</span></div></div>
      </div>
      <p className={s.companion}>Portnoy also picked Ohio State, which lost {ohio.score} at Texas. His tracked Week 2 underdog record was {portnoyDogs.filter(p => p.call.status === "hit").length}–{portnoyDogs.filter(p => p.call.status === "miss").length}. The Michigan hit belongs alongside <Link href={takePath(portnoyMiss.event.slug, portnoyMiss.pundit.id)}>the Ohio State miss ↗</Link></p>
    </section>

    <UnderdogTable report={r} />

    <section className={s.section} aria-labelledby="report-favorites">
      <div className={s.sectionHead}><h2 id="report-favorites">What the favorite record tells us</h2></div>
      <div className={s.prose}>
        <p>Texas supplied {texasHits} favorite hits; Alabama and Texas A&amp;M supplied the other {r.favorites.hits - texasHits}. At the game level, favorites went {favoriteWins}–{r.games.length - favoriteWins}. Oklahoma’s loss became {oklahoma.length} misses because that many tracked pundits backed the same team.</p>
        <p>This week covers four games, compared with eleven in our <Link href="/ncaaf/2026/week-1/">Week 1 report</Link>. A different slate and a small, selected sample cannot establish a trend in expert accuracy. The useful follow-up is whether these pundits keep identifying upsets as more of their picks settle.</p>
      </div>
      <Link className={s.nextLink} href="/ncaaf/">Follow the current college football picks <span>→</span></Link>
    </section>
    <ReportMethod report={r} />
