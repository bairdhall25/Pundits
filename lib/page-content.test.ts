...
describe("team page contract", () => {
  it("keeps the Melbourne result after 49ers-Rams grades", () => {
    ...
    expect(content.lede).toContain("49ers beat Dolphins");
    ...
    expect(content.historical[0]?.event.slug).toBe("dolphins-at-49ers-2026");
    expect(content.historical[0]?.beatLine).toMatch(/49ers beat Dolphins/);
    expect(content.historical.some((row) => row.event.slug === "49ers-vs-rams-2026")).toBe(true);
    expect(
      content.historical.find((row) => row.event.slug === "49ers-vs-rams-2026")?.beatLine
...
describe("league page contract", () => {
  it("keeps the NFL live week and points at the permanent archive", () => {
    ...
    expect(content.title).toBe("NFL Week 3: who picked whom");
    ...
    expect(content.lede).toMatch(/open game/);
    expect(content.title).not.toMatch(/best experts|expert picks/i);
    expect(content.description).toContain(TRACKED_SUBSET_DISCLAIMER);
  });
...
describe("weekly archive contract", () => {
  ...
  it("progresses NFL week 1 once the opener grades and keeps later games on the same URL", () => {
    ...
    expect(content.graded).toBe(true);
    expect(content.lede).toMatch(/Tracked Week 1 record: 10–11 on 21 graded picks\./);
