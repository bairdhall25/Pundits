...
The full repository check passes tests, type checking, build, and static-route verification, then stops on a route-specific social-card assertion for the separately added `/submit/` page. That route is linked from the navigation, but its social-card implementation is outside this navigation change. Production deployment remains pending explicit release approval.
...
- More menu:
  ...
  - About: Submit a source, About, Methodology, Contact
...
- Picks, College Football, NFL, Takes, Compact ledger, Pundits, Submit a source, About, Methodology, Contact, Privacy, and Terms are one-click footer destinations from every page.
