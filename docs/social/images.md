...
The site pre-renders a branded 1200×630 landscape card for every shareable route family. Events, takes, and pundits also receive 1080×1920 story cards. `cards.json` carries the exact URLs for events, takes, pundits, teams, weeks, and registered pages. Keep those cards and the current visual system. Phase 4 changes which stories get posted, not how the images look. Generated imagery is allowed only inside the Tier 2 brand spec below — nowhere else.
