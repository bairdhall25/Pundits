...
| Scout is a density engine: coordinator + Shows/X/News beats against homepage games. | Empty-YES Google against CFB YouTube factories produced dry mornings and ignored NFL/news. | Accepted 2026-08-29; amended 2026-09-08: density remains the display/coverage metric, not a stop on designated high-value sources for approved priority games. Hunt queue is `docs/capture-targets.json`. |
...
| Every Scout pass hunts NCAAF and NFL together. | Parking a sport for a calendar window missed weekday CFB and delayed NFL empty-YES. Skip ordinary dense hunting, not a sport; source-complete designated voices on approved priority games. | Accepted 2026-09-01; amended 2026-09-08 |
...
| Capture targets roll on a proposed/approved queue independent of `onHome`. | Launch-week watchlists went stale and left zero upcoming college events. Coordinator proposes 2–4 marquee games per sport; PM approves before hunting or minting. | Accepted 2026-09-08 |
| Audit disposition is row-specific. | A day-level fail blocked verified GameDay rows behind an unrelated overflow fail. Promote matches `rowId`; a changed quote cannot reuse approval. | Accepted 2026-09-08 |
...
| Homepage featured games use an ordered waterfall, not a score. | Pin, then when, then coverage (faces / both sides / disagreement), then size as a tie-break. No audience-timezone tweak. Empty side may sit in a section; it must not beat a two-sided open game for the hero. Home caps: 3 full cards and 2 compact teasers per sport, 2 latest Finals. League pages are live-week TV slates with the same full/compact coverage rule and no slot cap. See `docs/product/featured-games.md`. | Accepted 2026-09-01; amended 2026-09-03 |
...
| Pundit tags are earned by Roll Call, Flowers, or Milestone moments. | Tags should give a pundit a meaningful stage, specific credit, or tracked-record recognition—not notify them about every ledger update. Routine captures and misses remain untagged; handles come only from the approved social registry. See `docs/social/tagging.md`. | Accepted 2026-09-03 |
