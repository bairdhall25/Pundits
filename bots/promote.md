...
- `bots/scout-shows.md`, `bots/scout-x.md`, and `bots/scout-news.md` — what each hunter already verified (you do not re-mine)
- Latest Scout intake: `docs/runs/YYYY-MM-DD.md` on GitHub `main` (today, else the newest file). Include Intake rows from Shows, X, and News passes. Do not wait for a chat paste.
...
2. Match existing `calls.json` shape. `punditId` must exist. `eventSlug` must exist. `side` is `yes` or `no`. `kind: hard`, `status: pending`. Id pattern like `fallica-texas-cfp-20260825` (pundit-slug-date, unique). If Audit approved a non-empty Intake `reasoning` capsule, copy that source-grounded paraphrase into the optional call `reasoning` field exactly as staged. Do not generate, expand, or repair it during Promote; omit the field when blank.
