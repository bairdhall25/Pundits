...
6. Empty Intake → no-op. Say so. Do not touch `data/`.
   Exception: if Grader's latest grade file proposes deleting a **zero-pick** event that reached kickoff (docs/capture-policy.md rule 9), remove it from `events.json` in this pass. Only events with zero mapped calls, ever — a graded event is never deleted.
