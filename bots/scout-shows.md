...
3. Any qualifying named guest, fill-in or independent voice on those same shows (the add-list is a seed, not an allowlist) → Candidates (`photoUrl=needed` unless a real photo is already known).
...
Update the first-line `hard=` / `candidates=` counts (sum of new rows across passes). If you added or changed hard Intake or Candidates, set `audit=pending`, including candidate-only runs. Never set `promoted=true`. If the file was `promoted=true` and you added hard, set `promoted=false`. Set `## Lane status` Shows to `completed`, `dry`, `blocked`, or `not-run`. A connector or source-access failure is `blocked`, not a dry sweep. After source windows and bounded attempts, record a dry target and the next meaningful check. Honest empty sides are acceptable.
