...
- `bots/scout-shows.md`, `bots/scout-x.md`, and `bots/scout-news.md` evidence rules (season, YES = away, name the speaker). An X `sourceUrl` must be a status URL (`x.com/{handle}/status/{id}` or twitter.com). A radio source must be a durable episode, clip, transcript, or show-note URL that can be reopened.
