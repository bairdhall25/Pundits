...
7. Check `reasoning` independently from pick validity. A capsule must explain at most two concrete factors the same speaker gave for why that side should win in that source. It is not SU/ATS routing, quote-eligibility language, or an operator note. Blank reasoning is valid. Operational Scout notes, wager-category restatements (including SU vs ATS splits and “explicit winner language” as a category restatement), mixed capsules, copied captions, added analysis, out-of-range, unsupported, speaker-mixed, factually wrong, or multi-paragraph capsules are **capsule** defects, not pick defects: verdict `ok-no-reasoning` (or `ok-unmapped-no-reasoning`) and name the defect in the note. Example: Pate saying he separates Alabama SU from Kentucky ATS is routing, not why Alabama should win — omit the capsule (`ok-unmapped-no-reasoning` if unmapped); the pick may still be ok-unmapped. Never repair a capsule by inventing copy. A capsule defect never makes the row `fail`. Do not use a keyword filter as a substitute for source Audit.
...
Include `rowId` from `node -e` / `scripts/scout-handoff-lib.mjs` `rowIdentity` (evidence-v2: pundit + eventSlug + side + verbatim quote + sourceUrl + sourceDate + reasoning + targetId + matchup (legacy unmapped note if matchup is absent)). A changed quote or rationale is a new identity and cannot reuse the old approval.
