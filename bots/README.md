...
2. **Roster and events are live files**, not memory. Load `data/pundits.json` and `data/events.json` at the start of the job. Hunt order is `docs/board.md` (empty YES on home games before more futures). Hunt idle roster podcasts (Simmons, Kanell, McAfee-as-Pat, Florio, Simms) before any new outlet. Off-roster names never get an invented id. No Barstool/Ringer class expansion this week.
