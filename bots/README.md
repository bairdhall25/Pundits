...
Cadence is below (Coordinator daily; Shows/X/News on their calendars). Audit when new or changed hard Intake OR Candidates exist, including `hard=0 candidates>0`. Promote after Audit when the tally has at least one mapped row ready to promote. Do not ping a human to copy-paste the five blocks.
...
https://raw.githubusercontent.com/bairdhall25/Pundits/main/docs/capture-policy.md
...
10. **Capture the reason, not the transcript.** For a new hard pick, keep the decisive verbatim quote short, then add an optional `reasoning` capsule of at most 60 words that paraphrases at most two concrete factors the same speaker actually gave for why that side should win in the same source. A capsule is not SU/ATS routing, quote eligibility, or an operator note. No new analysis, generic filler, play-by-play, or transcript dump. Blank reasoning is valid. If the speaker gave only the pick, leave reasoning blank. Routing notes, including SU vs ATS splits and “explicit winner language” as a category restatement, go in `note`, never inside `reasoning`. `reasoning` is reader-facing copy and is the only field that can reach a pick page. Audit checks the capsule independently from pick validity; Promote omits a rejected capsule (`ok-no-reasoning` / `ok-unmapped-no-reasoning`) and does not reuse an approval after the rationale changes.
11. **Poster and Reply Guy never write to the repo.** Not `data/`, not `docs/`. They read the playbook (`docs/social/`) and `https://pundits.pro/social/cards.json`, and act on X only. Every number they post must be on pundits.pro at post time. They never repost third-party media and never AI-generate a real person's likeness. Novelty is live timeline/search keyed by canonical destination, call/event, and state across the pick/result lifecycle. Infer prior-post state from that post's own language: a predicted score is not a final result, and a later settled ledger must not relabel an old pregame post. If that coverage cannot be established, or a matching destination's state is unknown, skip rather than assume novelty. They do not write a publication log. Outbound X links append allowlisted campaign params; `cards.json` `pageUrl` and native site share stay canonical.
...
Candidate-only runs enter Audit and proposal preparation without waiting for mapped Intake or a Promote publication run. Audit writes candidate verdicts; Promote prepares the roster/photo decision packet even when there is nothing to publish. Coordinator surfaces unresolved packets in Dispatch. See docs/product/roster-growth.md.
